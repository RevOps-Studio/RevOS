...
**Requiere:** todos los outputs de la fase que se está cerrando. Por defecto, el checkpoint de Diagnostic requiere Client Master Brief, Knowledge Base, Competitive Landscape y Revenue Diagnostic.
...
· Activation (checkpoint ejecutivo final, nodo `activation-checkpoint`, con los outputs de `exec-deliverables` como material base) → cierre del proyecto tras validación del cliente y entrega final vía `/revos:entrega`
...
Enumera todos los outputs producidos en la fase. Para el checkpoint de Diagnostic serán típicamente 4: brief, knowledge base, competitive, revenue diagnostic. La agenda del checkpoint sale de la sección "Agenda del próximo checkpoint" del fichero `[Cliente] - Estado` (vista regenerada) — no de acumulados manuales.
...
[Lista de todas las [ASUNCIÓN] del Registro con su valor asumido y criterio. El cliente valida o corrige cada una. Las corregidas entran por el flujo de cambio (`/revos:cambio`) como cambio de Dato o de Concepto según afecte.]
...
4. Añade al final una sección **"Resumen para el consultor"** con:
   ...
   - Los hallazgos y las decisiones a tomar, listados — sin recuentos
