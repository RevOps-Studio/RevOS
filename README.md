...
## Estado: v4.4.0 · sistema completo, un piloto ejecutado

Arquitectura **skills-only**: 31 skills ejecutables (24 de producción + 7 de control) que implementan los 28 nodos del grafo (26 reales + 2 virtuales: `design-checkpoint` y `activation-checkpoint`), sin capa de comandos separada — las skills de control se invocan con interfaz de comando. La secuenciación por tier vive en el grafo del orquestador; el punto de entrada de cada sesión es `status`.
...
Piloto ejecutado: fase Diagnostic completa con cliente real (07/2026). Sus aprendizajes están en `docs/` y produjeron las versiones 4.1 y 4.2. La 4.3 es un parche de usabilidad tras dos iteraciones reales: doctrina de avance (bloqueante con definición cerrada, asunción de primera pasada), lenguaje calibrado y propagación en lote para cambios de Dato. La 4.3.1 es un parche de higiene derivado de la auditoría externa de agosto 2026: grafo alineado con los Requiere reales, enums canónicos de QA, alias corregidos y validador pre-empaquetado. La 4.4 rescata del programa v5 lo que tapa agujeros con daño observado —el método deja de recomendar un alcance que se firmó en pre-venta, el Resumen para el consultor no se renderiza al cliente, un resumen generado por un modelo no es fuente de evidencia, el checkpoint declara de qué se alimentó la fase— y deja fuera el paradigma. Histórico en `CHANGELOG.md`.
